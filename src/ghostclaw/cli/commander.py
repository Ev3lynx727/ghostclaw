...
class CommandRegistry:
    ...
    def register(self, command_class: Type[Command]) -> None:
        ...
        if not inspect.isclass(command_class) or not issubclass(command_class, Command):
            raise TypeError(
                f"Cannot register {command_class}: must be a subclass of Command"
            )
        ...
        try:
            ...
            if not name:
                raise ValueError(
                    f"Command class {command_class.__name__} must define a name property."
                )
    ...
    def auto_discover(self, package_name: str = "ghostclaw.cli.commands") -> None:
        ...
        try:
            ...
            print(
                f"Warning: Command package '{package_name}' could not be imported. Auto-discovery skipped. ({e})",
                file=sys.stderr,
            )
            return
        ...
        for _, module_name, is_pkg in pkgutil.walk_packages(
            ...
            try:
                ...
                print(
                    f"Warning: Failed to import command module {module_name}: {e}",
                    file=sys.stderr,
                )
