...
class AnalyzeCommand(Command):
    ...
    def configure_parser(self, parser: ArgumentParser) -> None:
        ...
        parser.add_argument(
            "--no-write-report",
            action="store_true",
            help="Skip writing the .md report file",
        )
        parser.add_argument(
            "--create-pr",
            action="store_true",
            help="Automatically create a GitHub PR with the report",
        )
        ...
        parser.add_argument(
            "--delta",
            action="store_true",
            help="Enable delta-context analysis (PR-style review on diffs)",
        )
        parser.add_argument(
            "--base",
            dest="delta_base_ref",
        ...
        parser.add_argument(
            "--delta-summary", action="store_true", help="Print diff statistics"
        )
        ...
        parser.add_argument(
            "--embedding-backend",
            choices=["sentence-transformers", "fastembed", "openai"],
        ...
        parser.add_argument(
            "--cache-ttl", type=int, default=7, help="Cache TTL in days"
        )
        parser.add_argument(
        ...
        # Parallel processing
        parser.add_argument(
            "--no-parallel", action="store_true", help="Disable parallel file scanning"
        )
        parser.add_argument(
            "--concurrency-limit", type=int, help="Max concurrent operations"
        )

        # AI options
        parser.add_argument("--use-ai", action="store_true", help="Enable AI synthesis")
        parser.add_argument(
            "--no-ai", action="store_true", help="Explicitly disable AI synthesis"
        )
        ...
        parser.add_argument(
            "--patch", action="store_true", help="Enable patch suggestions"
        )

        # Engine Integrations
        parser.add_argument(
            "--pyscn", action="store_true", help="Enable PySCN integration"
        )
        parser.add_argument(
            "--no-pyscn",
            action="store_true",
            help="Explicitly disable PySCN integration",
        )
        parser.add_argument(
            "--ai-codeindex",
            action="store_true",
            help="Enable AI-CodeIndex integration",
        )
        parser.add_argument(
            "--no-ai-codeindex",
            action="store_true",
            help="Explicitly disable AI-CodeIndex integration",
        )

        # Orchestrator
        orchestrator_group = parser.add_mutually_exclusive_group()
        orchestrator_group.add_argument(
            "--orchestrate", action="store_true", help="Enable orchestrator routing"
        )
        orchestrator_group.add_argument(
            "--no-orchestrate", action="store_true", help="Disable orchestrator routing"
        )
        ...
        parser.add_argument(
            "--orchestrate-llm",
            action="store_true",
            help="Enable LLM-based planning in orchestrator",
        )
        parser.add_argument(
            "--orchestrate-plan-only",
            action="store_true",
            help="Generate plan but do not execute plugins (debug/validation)",
        )
        parser.add_argument(
            "--orchestrate-max",
            type=int,
            help="Maximum number of plugins to run (overrides config)",
        )
        parser.add_argument(
            "--orchestrate-llm-model",
            type=str,
            help="LLM model to use for orchestrator planning (overrides config)",
        )
        parser.add_argument(
            "--orchestrate-concurrency",
            type=int,
            help="Concurrency limit for orchestrator plugin execution (overrides config)",
        )

        # Orchestrator caching (mutually exclusive)
        orchestrate_cache_group = parser.add_mutually_exclusive_group()
        orchestrate_cache_group.add_argument(
            "--orchestrate-cache",
            action="store_true",
            help="Enable plan caching in orchestrator",
        )
        orchestrate_cache_group.add_argument(
            "--no-orchestrate-cache",
            action="store_true",
            help="Disable plan caching in orchestrator",
        )

        # Reliability
        parser.add_argument(
            "--strict", action="store_true", help="Treat adapter errors as fatal"
        )

        # Observability
        parser.add_argument(
            "--benchmark", action="store_true", help="Print performance timings"
        )
        parser.add_argument("--pdb", action="store_true", help="Drop into pdb on error")
        parser.add_argument(
            "--show-tokens", action="store_true", help="Show token usage"
        )
    ...
    async def _execute_impl(self, args: Namespace) -> int:
        ...
        if args.create_pr:
            await self._handle_pr_creation(
                report, report_file_path, args.repo_path, args
            )

        self._print_auxiliary_info(report, service, args)
        if args.strict and report.get("errors"):
            print(
                f"\n\x1b[31m❌ Analysis failed due to {len(report['errors'])} adapter error(s) (--strict mode).\x1b[0m",
                file=sys.stderr,
            )
            return 1
        return 0
    ...
    def _handle_parallel_warning(
        self, args: Namespace, overrides: Dict[str, Any]
    ) -> None:
        ...
        if file_count > LARGE_REPO_THRESHOLD:
            ...
            print(
                f"⚡ Auto-enabling parallel processing (~{file_count} files).",
                file=sys.stderr,
            )
        else:
    ...
    async def _run_analysis(
        self, repo_path: str, cli_overrides: Dict[str, Any], args: Namespace
    ):
    ...
    async def _write_report(
        self, report: Dict[str, Any], args: Namespace
    ) -> Optional[Path]:
        now = datetime.datetime.now().strftime("%Y-%m-%dT%H-%M-%S")
        repo_path = Path(args.repo_path)
        is_delta = report.get("metadata", {}).get("delta", {}).get("mode", False)
        filename = (
            f"ARCHITECTURE-DELTA-{now}.md"
            if is_delta
            else f"ARCHITECTURE-REPORT-{now}.md"
        )

        report_dir = (
            repo_path
            if args.create_pr
            else (repo_path / ".ghostclaw" / "storage" / "reports")
        )
        ...
        try:
            report_file_path.write_text(
                MarkdownFormatter().format(report), encoding="utf-8"
            )
            print(
                f"📝 Report written to: {report_file_path.absolute()}", file=sys.stderr
            )
            report_file_path.with_suffix(".json").write_text(
    ...
    async def _handle_pr_creation(
        ...
        title = (
            args.pr_title
            or f"🏰 Architecture Report - {datetime.datetime.now().strftime('%Y-%m-%d')}"
        )
        body = (
            args.pr_body or f"Ghostclaw review. Vibe Score: {report['vibe_score']}/100"
        )
    ...
    def _print_auxiliary_info(
        self, report: Dict[str, Any], service, args: Namespace
    ) -> None:
