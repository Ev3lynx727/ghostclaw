...
class DebugCommand(Command):
    ...
    def configure_parser(self, parser: ArgumentParser) -> None:
        parser.add_argument(
            "repo_path", nargs="?", default=".", help="Path to repository"
        )
        parser.add_argument(
            "--break-at",
            choices=[
        ...
        parser.add_argument(
            "--no-cache", action="store_true", help="Disable caching for fresh analysis"
        )
        parser.add_argument(
            "--concurrency-limit", type=int, help="Override concurrency limit"
        )
    ...
    def validate(self, args: Namespace) -> None:
        ...
        debug_enabled = os.environ.get("GHOSTCLAW_DEBUG", "").lower() in (
            "1",
            "true",
            "yes",
        )
    ...
    async def execute(self, args: Namespace) -> int:
        ...
        print(
            "\x1b[36m[Debug] Starting analysis with breakpoint at: \x1b[33m"
            + args.break_at
            + "\x1b[0m"
        )
