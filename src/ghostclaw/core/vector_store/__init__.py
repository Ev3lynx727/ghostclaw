...
class VectorStore:
    ...
    def __init__(
        ...
        self.db_path = (
            db_path or Path.cwd() / ".ghostclaw" / "storage" / "qmd" / "lancedb"
        )
        self.embedding_backend = embedding_backend
        self._provider = EmbeddingProvider(
            backend=embedding_backend, cache_dir=embedding_cache_dir
        )
    ...
    async def initialize(self) -> None:
        ...
        self._index.connect()
        logger.info(
            "VectorStore initialized at %s with backend %s",
            self.db_path,
    ...
    async def search(
        ...
        if repo_path:
            where_clauses.append(
                f"repo_path = '{repo_path.replace(chr(39), chr(39) + chr(39))}'"
            )
        if stack:
            where_clauses.append(
                f"stack = '{stack.replace(chr(39), chr(39) + chr(39))}'"
            )
        if min_score is not None:
            where_clauses.append(f"vibe_score >= {int(min_score)}")
        if max_score is not None:
            where_clauses.append(f"vibe_score <= {int(max_score)}")
    ...
    async def search_by_run_id(
        self, report_id: int, limit: int = 10
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def add_chunks_for_report(
        self, report_id: int, report: Dict[str, Any], metadata: Dict[str, Any]
    ) -> None:
        """
        ...
        for issue in report.get("issues", []):
            chunks.append({"chunk_id": f"issue-{len(chunks)}", "text": str(issue)})
        # Ghosts
        for ghost in report.get("architectural_ghosts", []):
            chunks.append({"chunk_id": f"ghost-{len(chunks)}", "text": str(ghost)})
        # Flags
        for flag in report.get("red_flags", []):
            chunks.append({"chunk_id": f"flag-{len(chunks)}", "text": str(flag)})
        # AI synthesis
        ...
        if synthesis:
            ...
            for i, para in enumerate(paragraphs):
                if para.strip():
                    chunks.append({"chunk_id": f"synthesis-{i}", "text": para.strip()})
