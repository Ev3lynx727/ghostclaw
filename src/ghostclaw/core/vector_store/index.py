...
class VectorIndex:
    ...
    def ensure_table(self) -> None:
        ...
        if self._table is None:
            ...
            self._table = self._conn.create_table(
                "embeddings", schema=schema, mode="overwrite"
            )
    ...
    async def ensure_index(self) -> None:
        ...
        if index_type != "ivf_pq":
            logger.warning(
                "Unknown index type: %s; falling back to brute-force", index_type
            )
            return
        ...
        try:
            ...
            if total_rows < 1000:
                logger.warning(
                    "Insufficient training data for IVF-PQ (need >=1000 vectors, have %d); skipping index",
                    total_rows,
                )
                return
            ...
            if len(training_data) < 1000:
                logger.warning(
                    "Sampled data too small for training (%d rows)", len(training_data)
                )
                return
            ...
            logger.info(
                "Creating IVF-PQ index with %d partitions, %d sub_vectors, training on %d vectors",
                idx_cfg.get("partitions", 256),
                idx_cfg.get("sub_vectors", 64),
                len(train_vectors),
            )
            ...
            logger.error(
                "Failed to create IVF-PQ index: %s; falling back to brute-force", e
            )
    ...
    def search(
        self, query_vector: List[float], limit: int, where_clause: Optional[str] = None
    ) -> List[Dict[str, Any]]:
