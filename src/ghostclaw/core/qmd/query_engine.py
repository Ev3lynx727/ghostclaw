...
class QueryEngine:
    ...
    async def search(
        ...
        if use_hybrid:
            results = await self._hybrid_search(
                query, limit, repo_path, stack, min_score, max_score, alpha
            )
        elif self.fts and self.fts.is_initialized():
            results = await self.fts.search(
                query, limit, repo_path, stack, min_score, max_score
            )
        else:
    ...
    async def diff_runs(self, run_id_a: int, run_id_b: int) -> Dict[str, Any]:
        ...
        new_issues = [
            i for i in report_b.get("issues", []) if i not in report_a.get("issues", [])
        ]
        resolved_issues = [
            i for i in report_a.get("issues", []) if i not in report_b.get("issues", [])
        ]
        new_ghosts = [
            g
            for g in report_b.get("architectural_ghosts", [])
            if g not in report_a.get("architectural_ghosts", [])
        ]
        resolved_ghosts = [
            g
            for g in report_a.get("architectural_ghosts", [])
            if g not in report_b.get("architectural_ghosts", [])
        ]
        new_flags = [
            f
            for f in report_b.get("red_flags", [])
            if f not in report_a.get("red_flags", [])
        ]
        resolved_flags = [
            f
            for f in report_a.get("red_flags", [])
            if f not in report_b.get("red_flags", [])
        ]

        diff = {
            "vibe_score_delta": report_b.get("vibe_score", 0)
            - report_a.get("vibe_score", 0),
    ...
    async def knowledge_graph(self, limit: int = 50) -> Dict[str, Any]:
        ...
        for run in reversed(recent_runs):  # chronological
            ...
            score_trend.append(
                {"timestamp": run["timestamp"], "vibe_score": run["vibe_score"]}
            )
