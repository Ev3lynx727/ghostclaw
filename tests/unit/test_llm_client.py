...
async def test_generate_analysis_dry_run():
    ...
    with patch("ghostclaw.core.llm_client.AsyncOpenAI"), patch(
        "ghostclaw.core.llm_client.AsyncAnthropic"
    ):
...
def test_token_budget_exceeded(config):
    ...
    with pytest.raises(TokenBudgetExceededError):
        client._check_token_budget(
            "This is a very long prompt that will exceed the artificial ten token budget"
        )
...
async def test_retry_on_transient_failure():
    ...
    with patch("ghostclaw.core.llm_client.AsyncOpenAI"), patch(
        ...
        config = GhostclawConfig(
            retry_attempts=3, retry_backoff_factor=0.01, api_key="dummy"
        )
        client = LLMClient(config, ".")
...
async def test_retry_exhaustion_raises():
    ...
    with patch("ghostclaw.core.llm_client.AsyncOpenAI"), patch(
        ...
        config = GhostclawConfig(
            retry_attempts=2, retry_backoff_factor=0.01, api_key="dummy"
        )
        client = LLMClient(config, ".")
...
async def test_retry_stream_exhaustion():
    ...
    with patch("ghostclaw.core.llm_client.AsyncOpenAI"), patch(
        ...
        config = GhostclawConfig(
            retry_attempts=1, retry_backoff_factor=0.01, api_key="dummy"
        )
        client = LLMClient(config, ".")
